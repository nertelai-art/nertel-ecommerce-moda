...
select is((select count(*) from public.staff_catalog_variants()),3::bigint,'Catalog manager sees active and inactive variants');
select is((select count(*) from public.staff_categories()),2::bigint,'Catalog manager sees active and inactive categories');
select is((select count(*) from public.staff_product_categories()),3::bigint,'Catalog manager sees category assignments');
