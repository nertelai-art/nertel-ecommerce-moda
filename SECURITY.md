...
- Base SQL local amb RLS a totes les taules d'aplicació i proves d'accés, concurrència i integritat. Inventari, comandes, intents de pagament i permisos de personal viuen en esquema privat.
...
- Secrets exclosos de Git; l'exemple només declara noms de variables i cap valor.
- Les mutacions comercials només són executables pel rol secret del backend. PostgreSQL aplica límits distribuïts, instantànies de preu, idempotència vinculada al contingut i neteja programada.
...
No hi ha service worker ni caché offline: s'afegiran després amb llista permesa de recursos públics i proves de canvi d'usuari. No activar pagaments fins a verificar signatures de webhook i reconciliació; reserves i idempotència ja tenen proves.

Les comandes cancel·lades o caducades s'anonimitzen després de 90 dies. Els objectes d'imatge que no es poden retirar s'enregistren en una cua privada de neteja.
...
SSR amb cookies HttpOnly, validació d'origen, TOTP i permisos comprovats contra sessions actives. Les escriptures comercials passen exclusivament pel backend. L'estat actual i els controls pendents abans de producció són a docs/09-autenticacio.md.
